 ...
 *    + the list of generating values, which are used to create each net of our subspace. 
 ...
 *    + the size parameters, which are the parameters *common* to all nets which are needed by the construction rule.
 ...
 *    + The kernel based FOMs are computed thanks to the Lattice Builder machinery (with Coordinate Uniform evaluation). 
 * The important property of these FOMs is that the kernel \f$\omega\f$ is evaluated only on \f$n\f$ different points, which are \f${i/n, 0 <= i <= n}\f$. These \f$n\f$ values are stored.
 * Then computing the value of the FOM on a given projection amounts to clever (but simple) calculus on these $n$ values. 
 * The key point here is that on each coordinate, the point set is a permutation of \f${i/n, 0 <= i <= n}\f$, which is called a "Stride" in Lattice Builder.
 * This property is verified for general digital nets, as long as the generating matrices are square. 
 * The way to compute the Stride for digital nets is explained in more details in Section 4.3.2 of Maxime’s report (https://github.com/umontreal-simul/latnetbuilder/blob/dev/reports/report_godin.pdf),
 * and implemented in latbuilder/Storage-SIMPLE-DIGITAL.h.
 */ 
